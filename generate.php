...
if (empty($hostids) && empty($input_item_ids)) {
    ...
    if (GEN_DEBUG_TRACE) ztrace('hostids vacíos y tampoco llegaron itemids directos');
    exit;
}
...
if (!empty($input_item_keys) && !empty($hostids)) {
    foreach ($hostids as $hid) {
        ...
        if (empty($itemInfo)) { ztrace("Sin ítems en $hostName para la búsqueda exacta: " . json_encode($input_item_keys)); continue; }
...
try {
    ...
    // Limpieza de las imágenes PNG temporales
    if (!empty($entries)) {
